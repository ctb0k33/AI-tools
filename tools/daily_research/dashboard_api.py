...
DEFAULT_CONFIG = "tools/daily_research/selected_x_profiles.config.json"
...
def run_collection(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    config_path = project_path(payload.get("config"), DEFAULT_CONFIG)
    output_dir = project_path(payload.get("outputDir"), DEFAULT_OUTPUT_DIR)
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def handle_latest(self, query: str) -> None:
        ...
        output_dir = project_path(params.get("outputDir", [None])[0], DEFAULT_OUTPUT_DIR)
        try:
            digest, digest_path = load_digest(output_dir, target_date)
            self.send_json(
                {
